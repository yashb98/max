#!/usr/bin/env bash
#
# max-runtime-tunnel.sh — SSH tunnel helper for remote runtime HTTP access.
#
# Forwards a local TCP port to a remote Max runtime HTTP server via SSH.
# Designed for web local mode: set LOCAL_RUNTIME_URL to the forwarded port.
#
# Usage:
#   max-runtime-tunnel.sh start <ssh-host> [options]
#   max-runtime-tunnel.sh stop
#   max-runtime-tunnel.sh status
#   max-runtime-tunnel.sh print-env
#
# Options:
...
DEFAULT_PORT=7821
PID_FILE="${HOME}/.max/runtime-tunnel.pid"
INFO_FILE="${HOME}/.max/runtime-tunnel.info"
...
case "${1:-}" in
  ...
  *)
    ...
    echo "      Start an SSH tunnel to a remote Max runtime."
    echo "  stop"
