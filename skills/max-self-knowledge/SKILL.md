---
name: max-self-knowledge
description: Answer questions about Max, the assistant's architecture, capabilities, and current configuration by routing to live sources of truth
compatibility: "Designed for Max personal assistants"
metadata:
  emoji: "🪞"
  max:
    display-name: "Max Self-Knowledge"
    activation-hints:
      - "When the user asks what model the assistant is running on"
      - "When the user asks about Max, how the assistant works, or its architecture"
...
**Never answer from memory or general knowledge about Max.** Always go to a source of truth.
...
### 2. Max Docs Site — Conceptual Knowledge

For "what is", "how does", and "why" questions, fetch the relevant page from the docs site.
Base URL: `https://www.max.ai/docs`

| Topic                    | Path                                      |
| ------------------------ | ----------------------------------------- |
| What is Max           | `/getting-started/what-is-max`         |
...
2. The open source repo is at `https://github.com/max-ai/max-assistant`
3. The release for version X is at `https://github.com/max-ai/max-assistant/releases/tag/vX.Y.Z`
4. Check out the matching tag locally: `cd /workspace/max-assistant && git fetch --tags && git checkout v<version>`
