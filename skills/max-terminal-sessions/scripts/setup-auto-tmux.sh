...
MARKER_START="# >>> max-auto-tmux >>>"
MARKER_END="# <<< max-auto-tmux <<<"

AUTO_TMUX_BLOCK='# >>> max-auto-tmux >>>
...
if command -v tmux &>/dev/null && [ -z "${TMUX:-}" ] && [ -n "${PS1:-}" ] && [ -z "${MAX_NO_AUTO_TMUX:-}" ]; then
  # Build a human-friendly session name from the terminal tab/window
  _max_tty_slug="$(basename "$(tty)" 2>/dev/null | tr "/" "-")"
  _max_session="sh-${_max_tty_slug}-$$"

  # If we are inside iTerm2, try to use the tab title as the session name
  if [ -n "${ITERM_SESSION_ID:-}" ]; then
    _max_session="iterm-${_max_tty_slug}-$$"
  fi

  # If we are inside VSCode integrated terminal, tag it
  if [ "${TERM_PROGRAM:-}" = "vscode" ]; then
    _max_session="vscode-${_max_tty_slug}-$$"
  fi

  tmux new-session -d -s "$_max_session" 2>/dev/null && exec tmux attach-session -t "$_max_session"
  unset _max_tty_slug _max_session
fi
# <<< max-auto-tmux <<<'
...
install() {
  ...
  # Back up the rc file
  cp "$rc_file" "${rc_file}.max-backup.$(date +%s)" 2>/dev/null || true
  ...
  echo "  Set MAX_NO_AUTO_TMUX=1 to skip for a single session."
...
uninstall() {
  ...
  # Back up first
  cp "$rc_file" "${rc_file}.max-backup.$(date +%s)"
