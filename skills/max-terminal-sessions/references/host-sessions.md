...
**Install it** by copying to somewhere on the user's PATH. Replace `<skill-dir>` with the actual path to this skill directory (e.g. the workspace `skills/max-terminal-sessions` path):
...
The auto-created session names include the terminal app context (`iterm-`, `vscode-`, or `sh-`) plus the TTY and PID for uniqueness. The user can skip auto-tmux for a single shell by setting `MAX_NO_AUTO_TMUX=1`.
