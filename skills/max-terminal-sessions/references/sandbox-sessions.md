...
In **Sandbox Mode**, the assistant runs tmux sessions inside its own sandbox environment. The user can attach to any of these sessions live via the Max terminal UI.
...
3. The user can observe any session in real time by running:
   ...
   ```
   max terminal attach <session-name>
   ```

   > **Note:** `max terminal attach` requires a managed (cloud-hosted) assistant. For local/Docker setups, use `tmux attach -t <session-name>` directly.

   This opens a live, read-write terminal view of that session in the Max UI.
...
# User attaches to watch live output in Max terminal:
# max terminal attach frontend
```
