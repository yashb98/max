...
The included `max-oauth-integrations` skill handles the generic parts of the flow (credential collection, app registration, connection, and verification). This file defines only the Google-specific steps.
...
**New project:** Open `https://console.cloud.google.com/projectcreate` -> name it `max-assistant` -> click Create -> get the project ID.
...
>
> **Step 1 - App Information:** App name: `Max Assistant`, leave the rest
...
If needs setup: fill in App name (`Max Assistant`), User support email, Developer contact email -> Save. If already filled, skip to Step 5b.
...
>
> If you see **"This app isn't verified"**, click **Advanced** then **Go to Max Assistant (unsafe)**. This is normal for apps in testing mode.
>
