...
metadata:
  emoji: "🔌"
  max:
    display-name: "Max OAuth Integrations"
---
...
Max assistants can natively integrate with any application that supports OAuth 2.0. Many OAuth providers come pre-configured and ready to use. You can view them by running:
...
"managed" mode relies on a first-class integration with the Max Platform. Managed mode is typically easier to set up and get going, often only requiring the user to log in with no additional configuration needed before they can begin using the integration. Managed mode is the recommended method for most users, especially those that are less technical or newer to their Max assistant.

Note that using managed mode:

- Requires an account with the Max Platform
- May result in billable usage
- Requires that requests to the third party are sent through Max's servers

### Your-Own Mode

"your-own" mode requires that the user creates their own OAuth application directly with the third-party and then enter the application's Client ID and Client Secret into Max.

Your-own mode is typically best if:

- Max does not have a first-party integration with the provider and managed mode is not supported
- The user is more tech-savvy and comfortable setting up OAuth apps
- The user does not want to create an account with the Max Platform
- The user is more sensitive to potential billing implications
- The user is sensitive to their data going to the Max Platform
...
- **[Registering New OAuth Providers](references/REGISTERING_PROVIDERS.md)** - How to register a new OAuth provider that doesn't come with Max's defaults
