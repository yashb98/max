...
**All commands in this skill use the `bash` tool.** `$MAX_WORKSPACE_DIR` is available in the sandbox environment — do not use `host_bash`.

## What you're configuring

Two stores, both under `$MAX_WORKSPACE_DIR/data/sounds/`:
...
```bash
ls "$MAX_WORKSPACE_DIR/data/sounds/" 2>/dev/null || echo "No sounds directory yet"
cat "$MAX_WORKSPACE_DIR/data/sounds/config.json" 2>/dev/null || echo "No config yet"
```
...
```bash
mkdir -p "$MAX_WORKSPACE_DIR/data/sounds"
cp "<source-path>" "$MAX_WORKSPACE_DIR/data/sounds/<descriptive-name>.<ext>"
```
...
```bash
rm "$MAX_WORKSPACE_DIR/data/sounds/<filename>"
```
