#!/usr/bin/env bun
/**
 * CLI for max-sounds skill: `bun run scripts/update-config.ts`
 *
 * Safely reads and writes $MAX_WORKSPACE_DIR/data/sounds/config.json,
...
function resolveSoundsDir(): string {
  const workspace = process.env.MAX_WORKSPACE_DIR;
  if (!workspace) {
    fail(
      "MAX_WORKSPACE_DIR is not set. This script must run inside the Max skill sandbox.",
    );
  }
