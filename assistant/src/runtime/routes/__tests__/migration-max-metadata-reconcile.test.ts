/**
 * Tests for the post-import max metadata reconciliation helper.
 *
 * After every bundle import, `reconcileMaxMetadataFromCes` walks the
...
const { reconcileMaxMetadataFromCes } =
  (await import("../migration-routes.js")) as unknown as {
    reconcileMaxMetadataFromCes: (sink: {
      warnings: string[];
    }) => Promise<void>;
  };
...
function seedAllInCes(): void {
  for (const field of MAX_FIELDS) {
    cesValues.set(credentialKey("max", field), `value-for-${field}`);
  }
}

describe("reconcileMaxMetadataFromCes", () => {
  test("CES holds all fields, metadata empty → all upserted", async () => {
    seedAllInCes();
    const sink = { warnings: [] as string[] };

    await reconcileMaxMetadataFromCes(sink);

    expect(upsertCalls).toHaveLength(MAX_FIELDS.length);
  ...
  test("CES holds all, metadata has 2 → only the missing ones upserted", async () => {
    ...
    for (const field of prepopulated) {
      metadataStore.set(`max:${field}`, {
        credentialId: `id-max:${field}`,
        service: "max",
        field,
        allowedTools: [],
        allowedDomains: [],
    ...
    const expectedMissing = MAX_FIELDS.filter(
  ...
  test("upsert throws for one field → warning recorded, loop continues", async () => {
    ...
    expect(calls).toBe(MAX_FIELDS.length);
    expect(sink.warnings).toHaveLength(1);
    expect(sink.warnings[0]).toContain("max:assistant_api_key");
  });
});
