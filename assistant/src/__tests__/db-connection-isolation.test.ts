...
const originalAllowRealWorkspace =
  process.env.MAX_ALLOW_REAL_WORKSPACE_IN_TESTS;
const originalTestRealWorkspace = process.env.MAX_TEST_REAL_WORKSPACE_DIR;
...
afterEach(() => {
  ...
  if (originalAllowRealWorkspace === undefined) {
    delete process.env.MAX_ALLOW_REAL_WORKSPACE_IN_TESTS;
  } else {
    process.env.MAX_ALLOW_REAL_WORKSPACE_IN_TESTS =
  ...
  if (originalTestRealWorkspace === undefined) {
    delete process.env.MAX_TEST_REAL_WORKSPACE_DIR;
  } else {
    process.env.MAX_TEST_REAL_WORKSPACE_DIR = originalTestRealWorkspace;
  }
...
test("getDb refuses the real workspace during tests even when explicitly set", () => {
  resetDb();
  process.env.MAX_WORKSPACE_DIR = join(homedir(), ".max", "workspace");
  delete process.env.MAX_ALLOW_REAL_WORKSPACE_IN_TESTS;
...
test("getDb refuses symlink aliases to the real workspace during tests", () => {
  ...
  const testRoot = realpathSync(
    mkdtempSync(join(tmpdir(), "max-db-isolation-")),
  );

  try {
    const fakeHome = join(testRoot, "home");
    const realWorkspace = join(fakeHome, ".max", "workspace");
    ...
    process.env.MAX_WORKSPACE_DIR = workspaceAlias;
    process.env.MAX_TEST_REAL_WORKSPACE_DIR = realWorkspace;
    delete process.env.MAX_ALLOW_REAL_WORKSPACE_IN_TESTS;
...
test("getDb refuses missing children under symlink aliases to the real workspace", () => {
  ...
  try {
    ...
    process.env.HOME = fakeHome;
    process.env.MAX_WORKSPACE_DIR = missingChild;
    process.env.MAX_TEST_REAL_WORKSPACE_DIR = realWorkspace;
    delete process.env.MAX_ALLOW_REAL_WORKSPACE_IN_TESTS;
