/**
 * Tests that max-self-knowledge loads correctly and contains the expected
...
/** Copy the real max-self-knowledge SKILL.md into the test skills dir. */
function installSelfKnowledgeSkill(): void {
  const destDir = join(TEST_DIR, "skills", "max-self-knowledge");
...
describe("max-self-knowledge skill", () => {
  beforeEach(() => {
  ...
  test("loads without error", async () => {
    const result = await executeSkillLoad({ skill: "max-self-knowledge" });
  ...
  test("references the docs site for conceptual questions", async () => {
    const result = await executeSkillLoad({ skill: "max-self-knowledge" });
    expect(result.content).toContain("https://www.max.ai/docs");
    expect(result.content).toContain("/getting-started/what-is-max");
  ...
  test("references the GitHub repo for deep implementation questions", async () => {
    const result = await executeSkillLoad({ skill: "max-self-knowledge" });
    expect(result.content).toContain("github.com/max-ai/max-assistant");
