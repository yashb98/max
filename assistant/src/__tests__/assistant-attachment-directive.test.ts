...
const RAW_TEST_DIR = join(tmpdir(), `max-sandbox-test-${Date.now()}`);
...
describe("parseDirectives", () => {
  test("parses a single sandbox directive with all attributes", () => {
    const text =
      'Here is the report:\n<max-attachment source="sandbox" path="output/report.pdf" filename="report.pdf" mime_type="application/pdf" />';
  ...
  test("defaults source to sandbox when omitted", () => {
    const text = '<max-attachment path="chart.png" />';
    const result = parseDirectives(text);
  ...
  test("parses host source", () => {
    const text = '<max-attachment source="host" path="/Users/me/doc.pdf" />';
    const result = parseDirectives(text);
  ...
  test("leaves optional filename and mime_type undefined when absent", () => {
    const text = '<max-attachment path="file.txt" />';
    const result = parseDirectives(text);
  ...
  test("parses multiple directives preserving order", () => {
    const text = [
      "Results:",
      '<max-attachment path="a.png" />',
      "And also:",
      '<max-attachment path="b.pdf" />',
    ].join("\n");
  ...
  test("rejects directive without path attribute", () => {
    const text = '<max-attachment source="sandbox" />';
    const result = parseDirectives(text);
    ...
    expect(result.cleanText).toContain("<max-attachment");
  });

  test("rejects directive with invalid source value", () => {
    const text = '<max-attachment source="cloud" path="x.txt" />';
    const result = parseDirectives(text);
  ...
  test("handles mixed valid and invalid directives", () => {
    const text = [
      '<max-attachment path="good.png" />',
      '<max-attachment source="nope" path="bad.txt" />',
      '<max-attachment path="also-good.pdf" />',
    ].join("\n");
  ...
  test("preserves non-self-closing tags as plain text", () => {
    const text =
      '<max-attachment path="file.txt">content</max-attachment>';
    const result = parseDirectives(text);

    // The regex only matches self-closing tags, so non-self-closing is not matched
    expect(result.directiveRequests).toHaveLength(0);
    expect(result.cleanText).toContain("content</max-attachment>");
  });

  test("handles single-quoted attributes", () => {
    const text =
      "<max-attachment path='report.pdf' filename='my report.pdf' />";
    const result = parseDirectives(text);
