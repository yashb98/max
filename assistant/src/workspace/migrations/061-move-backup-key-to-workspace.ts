 ...
 *
 * The backup encryption key previously lived at ~/.max/protected/backup.key.
 * This migration copies it to ~/.max/workspace/.backup.key so the daemon
 ...
 * exists (e.g. from MAX_BACKUP_KEY_PATH override or re-run), the old
...
export const moveBackupKeyToWorkspaceMigration: WorkspaceMigration = {
  ...
  run(workspaceDir: string): void {
    const oldPath = join(getMaxRoot(), "protected", "backup.key");
