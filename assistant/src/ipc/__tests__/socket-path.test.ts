...
describe("resolveIpcSocketPath", () => {
  test("uses env var override when set", () => {
    ...
    process.env.MAX_WORKSPACE_DIR = "/tmp/max-workspace-test";
  ...
  test("ignores empty env var override", () => {
    ...
    expect(resolved.path).toBe("/tmp/max-workspace-test/gateway.sock");
  });
  ...
  test("falls back to tmpdir when workspace path exceeds AF_UNIX limit", () => {
    ...
    process.env.MAX_WORKSPACE_DIR = "/tmp/" + "a".repeat(85) + "/workspace";
