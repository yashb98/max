...
/// Runs the Max stack inside a single `LinuxPod` VM.
///
...
final class AppleContainersPodRuntime: @unchecked Sendable {
    ...
    struct Configuration: Sendable {
        ...
        var serviceImageRefs: [MaxServiceName: String]
        var instanceDir: URL
    ...
    func start(progress: @escaping KataKernelStore.ProgressHandler) async throws {
        ...
        let results = try await withThrowingTaskGroup(
            of: (MaxServiceName, Containerization.Mount, ContainerizationOCI.ImageConfig?).self
        ) { group in
            for service in MaxServiceName.startOrder {
            ...
            var rootfs: [MaxServiceName: Containerization.Mount] = [:]
            var configs: [MaxServiceName: ContainerizationOCI.ImageConfig?] = [:]
        ...
        let sharedMounts: [Containerization.Mount] = [
            .share(source: workspaceDir.path, destination: MaxMountPaths.workspace),
            .share(source: cesBootstrapDir.path, destination: MaxMountPaths.cesBootstrap),
            .share(source: gatewayIpcDir.path, destination: MaxMountPaths.gatewayIpcSocketDir),
            .share(source: assistantIpcDir.path, destination: MaxMountPaths.assistantIpcSocketDir),
        ]
        ...
        // Assistant
        let assistantEnv = MaxContainerEnv.assistant(
        ...
        // Gateway
        let gatewayEnv = MaxContainerEnv.gateway(
            signingKey: config.signingKey,
        ...
        try await pod.addContainer(
            ...
            c.mounts = sharedMounts + [
                .share(source: gatewaySecurityDir.path, destination: MaxMountPaths.gatewaySecurityDir),
            ] + LinuxContainer.defaultMounts()
        }

        // Credential Executor — workspace mounted read-only to match Docker topology.
        let cesEnv = MaxContainerEnv.credentialExecutor(
            cesServiceToken: config.cesServiceToken
        )
        try await pod.addContainer(
            ...
            c.mounts = [
                .share(source: workspaceDir.path, destination: MaxMountPaths.workspace, options: ["ro"]),
                .share(source: cesBootstrapDir.path, destination: MaxMountPaths.cesBootstrap),
                .share(source: cesSecurityDir.path, destination: MaxMountPaths.cesSecurityDir),
            ] + LinuxContainer.defaultMounts()
        }
        ...
        let gatewayURL = "http://\(podIP):\(MaxContainerPorts.gatewayHTTP)"
        lock.withLock {
            _pod = pod
            _network = network
    ...
    func exec(
        service: MaxServiceName = .assistant,
    ...
    enum PodRuntimeError: LocalizedError {
        case missingImageRef(MaxServiceName)
    ...
    private func containerID(_ service: MaxServiceName) -> String {
