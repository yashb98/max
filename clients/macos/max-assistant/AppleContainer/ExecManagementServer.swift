...
///    `{"command": ["/bin/bash"], "service": "max-assistant", "cols": 120, "rows": 40}\n`
...
final class ExecManagementServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.max.mgmt-socket", qos: .userInitiated)
    ...
    private func parseExecRequest(_ json: [String: Any]) -> ExecRequest? {
        ...
        let serviceName = (json["service"] as? String) ?? MaxServiceName.assistant.rawValue
        let service = MaxServiceName(rawValue: serviceName) ?? .assistant
