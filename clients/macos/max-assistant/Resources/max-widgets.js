/* ============================================================
   Max Widgets — Interactive JS Utilities
   Injected alongside max-design-system.css into all
   dynamic page WKWebViews via WKUserScript.

   API: window.max.widgets.*
   ============================================================ */
  ...
  widgets.multiSelect = function (tableId) {
    ...
    if (selectAll) {
      selectAll.addEventListener('change', function () {
        ...
        if (window.max && window.max.sendAction) {
          window.max.sendAction('multiSelect', { tableId: tableId, selected: getSelected() });
        }
      });
    }
