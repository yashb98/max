/* ============================================================
   max-edit-animator.js
   DOM diffing and animated mutation engine for Max Assistant.
   Injected into dynamic page WKWebViews at document end.

   API: window.max.morphWithAnimation(newHTML) → Promise<{success, fallback, cancelled}>
   ============================================================ */

(function () {
  'use strict';

  // Extend the existing window.max namespace (created by the bridge script).
  if (!window.max) window.max = {};

  // Cancellation generation counter.
  window.max._morphGen = 0;

  // ─── Companion CSS ─────────────────────────────────────────────
  (function injectStyles() {
    var style = document.createElement('style');
    style.setAttribute('data-max-injected', 'true');
    style.textContent = [
      '.max-cursor {',
      '  display: inline;',
      '  animation: max-blink 0.6s step-end infinite;',
      '  color: var(--v-accent, #537D53);',
      '}',
      '@keyframes max-blink { 50% { opacity: 0; } }'
    ].join('\n');
  ...
  function isMaxInjected(node) {
    return node.nodeType === 1 && node.hasAttribute('data-max-injected');
  }
  ...
  async function animateTextReplace(op, cancelled) {
    ...
    wrapper.setAttribute('data-max-injected', 'true');
    ...
    cursor.className = 'max-cursor';
    cursor.setAttribute('data-max-injected', 'true');
  ...
  window.max.morphWithAnimation = async function (newHTML) {
    var gen = ++window.max._morphGen;
    var cancelled = function () { return window.max._morphGen !== gen; };
