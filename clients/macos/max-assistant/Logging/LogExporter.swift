...
/// - `~/Library/Application Support/max-assistant/logs/`  — per-session JSONL diagnostic logs
/// - `~/Library/Application Support/max-assistant/debug-state.json` — live debug snapshot (includes transcript diagnostics)
/// - `~/Library/Application Support/max-assistant/hang-context.json` — hang diagnostic context written during main-thread stalls
/// - `~/Library/Application Support/max-assistant/hang-sample*.txt` — process sample captures from prolonged stalls
...
/// - `~/.config/max/logs/` — CLI XDG logs (hatch.log, retire.log, etc.)
/// - `~/.max.lock.json` — sanitized lockfile with assistant entries and resource ports (credentials stripped)
...
/// - `crash-reports/` — recent macOS crash/hang reports (.ips, .crash, .spin) for assistant-related processes (bun, qdrant, max-assistant)
...
enum LogExporter {
    ...
    static func sendFeedback(formData: LogReportFormData) async throws {
        ...
        let stagingDir = fileManager.temporaryDirectory
            .appendingPathComponent("max-log-export-\(UUID().uuidString)", isDirectory: true)
        let archiveURL = fileManager.temporaryDirectory
            .appendingPathComponent("max-assistant-logs-\(UUID().uuidString).tar.gz")
    ...
    private static func defaultArchiveName() -> String {
        ...
        return "max-assistant-logs-\(timestamp).tar.gz"
    }
    ...
    private nonisolated static func populateStagingDirectory(
        ...
        if shouldCollectLogs {
            ...
            if let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
                let appSupportDir = appSupport.appendingPathComponent(MaxEnvironment.current.appSupportDirectoryName, isDirectory: true)
            ...
            // 4. XDG CLI logs — ~/.config/max/logs/ (hatch.log, retire.log, etc.)
            ...
            let xdgLogDir = URL(fileURLWithPath: xdgConfigHome)
                .appendingPathComponent("max/logs", isDirectory: true)
            ...
            // 5. Lockfile — ~/.max.lock.json (sanitized to strip credentials)
            writeSanitizedLockfile(
                to: tempDir.appendingPathComponent("max.lock.json")
            )
        ...
        if let formData {
            var metadata: [String: Any] = [
                ...
                // device_id intentionally matches ~/.max/device.json UUID
    ...
    /// `max-assistant/` directory into the export staging directory.
    ///
    /// Artifacts collected:
    ...
    private nonisolated static func fetchPlatformLogs(
        ...
        request.timeoutInterval = 60
        request.setValue(token, forHTTPHeaderField: "X-Session-Token")
        request.setValue(organizationId, forHTTPHeaderField: "Max-Organization-Id")
