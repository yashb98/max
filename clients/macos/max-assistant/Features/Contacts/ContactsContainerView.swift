    ...
    private func guardianDetailView(contact: ContactPayload) -> some View {
        ScrollView {
            VStack(alignment: .leading, spacing: VSpacing.lg) {
                ...
                SettingsCard(
                    title: contact.displayName.hasPrefix("max-principal-") ? "You" : "\(contact.displayName) (You)",
                    ...
                    HStack(spacing: VSpacing.sm) {
                        VButton(
                            label: "Save",
                            style: .primary,
                            isDisabled: guardianIsSaving || (guardianEditedName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty && !contact.displayName.hasPrefix("max-principal-"))
                        ) {
        ...
        .id(contact.id)
        .onAppear {
            guardianEditedName = contact.displayName.hasPrefix("max-principal-") ? "" : contact.displayName
        ...
        .onChange(of: contact) { _, newContact in
            ...
            guardianEditedName = newContact.displayName.hasPrefix("max-principal-") ? "" : newContact.displayName
    ...
    private func saveGuardianEdits(contact: ContactPayload) async {
        ...
        do {
            if let updated = try await contactClient.updateContact(
                ...
                guardianEditedName = updated.displayName.hasPrefix("max-principal-") ? "" : updated.displayName
