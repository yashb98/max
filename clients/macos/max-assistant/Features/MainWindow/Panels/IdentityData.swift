...
enum AssistantHome: Equatable {
    ...
    static func parse(_ raw: String) -> AssistantHome? {
        ...
        if lower.hasPrefix("local") {
            ...
            let path = detail ?? NSHomeDirectory() + "/.max/workspace"
...
extension LockfileAssistant {
    var home: AssistantHome {
        ...
        case "max":
            return .max(runtimeUrl: runtimeUrl ?? "")
        ...
        default:
            ...
            return .local(workspacePath: base + "/.max/workspace")
        }
    }
    ...
    func loadDisplayName() -> String? {
        ...
        let identityPath = base + "/.max/workspace/IDENTITY.md"
