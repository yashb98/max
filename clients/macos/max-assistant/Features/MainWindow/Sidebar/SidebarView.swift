...
struct SidebarView: View {
    ...
    private var expandedSidebarContent: some View {
        VStack(spacing: SidebarLayoutMetrics.listRowGap) {
            ...
            .onGeometryChange(for: CGFloat.self) { $0.size.height } action: { sidebarFrameHeight = $0 }
...
// Scroll-overflow detection (content height vs. viewport height) is observed via
// `.onGeometryChange` in `expandedSidebarContent`. A previous implementation used
// `GeometryReader`-in-`.background` + `PreferenceKey` + `onPreferenceChange`, which
// fed measured heights back into `@State` *during* the layout transaction and could
// drive SwiftUI's flex-frame layout into a non-terminating recursion (main-thread
// hang). `.onGeometryChange` observes geometry outside that preference-reconciliation
// path, matching the pattern used elsewhere (e.g. `SubagentDetailPanel`).
