...
final class BundleConfirmationViewModel {
    ...
    /// The app icon extracted from the .max ZIP, or a fallback emoji rendering.
    ...
    /// Attempts to extract icon.png from the .max ZIP. Falls back to emoji rendering.
    ...
    /// Extract icon.png from a .max ZIP without fully unpacking.
