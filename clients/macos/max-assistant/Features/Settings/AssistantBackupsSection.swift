...
extension AssistantBackupsSection {
    ...
    private func handleRestoreSuccess(message: String) {
        ...
        Task {
            if isDocker {
                try? await AppDelegate.shared?.maxCli.sleep(name: assistantName)
            } else {
                await AppDelegate.shared?.maxCli.stop(name: assistantName)
            }
            try? await Task.sleep(nanoseconds: 500_000_000)
            try? await AppDelegate.shared?.maxCli.wake(name: assistantName)
