...
enum AssistantTopology {
    ...
    case managed     // Platform-managed (Max cloud)
...
struct AssistantUpgradeSection: View {
    ...
    private func loadReleasesQuietly() async {
        ...
        let platformBase = MaxEnvironment.resolvedPlatformURL
    ...
    private func performDockerUpgrade() async {
        guard let cli = AppDelegate.shared?.maxCli else {
        ...
        } catch let error as MaxCli.CLIError {
            switch error {
    ...
    ///
    /// The CLI path (`max upgrade <id>`) fails for managed assistants
    ...
    private func guidanceForError(_ error: MaxCli.CliError) -> String {
