...
struct ProvidersSheet: View {
        ...
        HStack(alignment: .top, spacing: VSpacing.md) {
            ...
            VStack(alignment: .leading, spacing: VSpacing.xs) {
                Text(title)
                    ...
                    .foregroundStyle(VColor.contentDefault)
                if isAuthLocked {
                    Text("Managed by Max — auth is locked, but you can rename or disable this connection.")
                        .font(VFont.bodySmallDefault)
                        .foregroundStyle(VColor.contentSecondary)
                }
            }
    ...
    private var authTypeOptions: [(label: String, value: String)] {
        ...
        if store.isPlatformCapable(provider) {
            options.append((label: "Platform (managed by Max)", value: "platform"))
        }
        ...
        if !current.isEmpty && !options.contains(where: { $0.value == current }) {
            ...
            case "platform": "Platform (managed by Max)"
    ...
    private var editorPlatformNote: some View {
        HStack(spacing: VSpacing.sm) {
            VIconView(.info, size: 16)
                .foregroundStyle(VColor.contentSecondary)
            Text("Managed by Max — no additional credentials required.")
                .font(VFont.bodySmallDefault)
                .foregroundStyle(VColor.contentSecondary)
        }
        .padding(VSpacing.md)
    ...
    /// Open the editor for a Max-managed connection. Auth-related fields
