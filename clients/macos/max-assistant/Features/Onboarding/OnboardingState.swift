...
final class OnboardingState {
    ...
    var selectedHostingMode: HostingMode = .maxCloud
    ...
    enum HostingMode: String {
        ...
        var subtitle: String {
            switch self {
            case .maxCloud: return "Always on, 24/7, even when your Mac is asleep. Runs on Max's secure infrastructure."
    ...
    // Cloud credentials held in memory during onboarding (never written to .max)
    ...
    /// `max_provider_*` credential files. The daemon is the only durable
