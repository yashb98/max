...
final class AvatarAppearanceManager {
    ...
    func resetForDisconnect() {
        ...
        // falls back to the env-default ("Max"/"Max Dev"). `updateDockLabel`
    ...
    /// to "restore" it instead of the real Max logo. Reading the `.icns`
    ...
    /// Restores the dock icon to the default Max logo.
    ///
    ...
    /// avatar rendered from saved traits, then restores the bundled Max logo.
    ///
    ...
    /// `~/.config/max/dock-display-name` for production,
    /// `~/.config/max-dev/dock-display-name` for dev).
    private static let dockDisplayNameURL: URL = {
        MaxPaths.current.configDir
    ...
    private func updateDockLabel() {
        // Only persist a resolved persona name. Writing a "Max" fallback
        ...
        // point causes the next rebuild's `build.sh` to read "Max" and
        // produce `Max.app` alongside the existing persona-named bundle.
