    ...
    class Coordinator: NSObject, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
        ...
        func userContentController(
            ...
            if let type = body["type"] as? String, type == "fetch_request" {
                if sandboxMode {
                    ...
                    if let callId = body["callId"] as? String {
                        ...
                        webView?.evaluateJavaScript("window.max._rejectFetch('\(safeCallId)', 'Request blocked: sandbox mode')", completionHandler: nil)
                    }
                    return
                }
                guard let callId = body["callId"] as? String,
                ...
                guard let url = URL(string: urlString) else {
                    log.error("[max.fetch] Invalid URL: \(urlString, privacy: .public)")
                    let safeCallId = callId
                        .replacingOccurrences(of: "\\", with: "\\\\")
                        .replacingOccurrences(of: "'", with: "\\'")
                        .replacingOccurrences(of: "\n", with: "\\n")
                        .replacingOccurrences(of: "\r", with: "\\r")
                    webView?.evaluateJavaScript("window.max._rejectFetch('\(safeCallId)', 'Invalid URL')", completionHandler: nil)
                    return
                }
                ...
                guard let allowed = allowedFetchBaseURL, urlString.hasPrefix(allowed + "/") else {
                    log.error("[max.fetch] Blocked request to disallowed origin: \(urlString, privacy: .public) (allowed: \(self.allowedFetchBaseURL ?? "nil", privacy: .public))")
                    let safeCallId = callId
                        .replacingOccurrences(of: "\\", with: "\\\\")
                        .replacingOccurrences(of: "'", with: "\\'")
                        .replacingOccurrences(of: "\n", with: "\\n")
                        .replacingOccurrences(of: "\r", with: "\\r")
                    webView?.evaluateJavaScript("window.max._rejectFetch('\(safeCallId)', 'Request blocked: disallowed origin')", completionHandler: nil)
                    return
                }
                ...
                guard isCustomRoute else {
                    log.error("[max.fetch] Blocked request to disallowed path: \(canonicalPath, privacy: .public)")
                    let safeCallId = callId
                        .replacingOccurrences(of: "\\", with: "\\\\")
                        .replacingOccurrences(of: "'", with: "\\'")
                        .replacingOccurrences(of: "\n", with: "\\n")
                        .replacingOccurrences(of: "\r", with: "\\r")
                    webView?.evaluateJavaScript("window.max._rejectFetch('\(safeCallId)', 'Request blocked: path not allowed')", completionHandler: nil)
                    return
                }
                ...
                Task.detached(priority: .userInitiated) {
                    do {
                        ...
                        let js = "window.max._resolveFetch('\(safeCallId)', \(statusCode), '\(safeStatusText)', '\(escapedBody)')"
                        await MainActor.run {
                        ...
                        let errorMessage = error.localizedDescription
                            ...
                            .replacingOccurrences(of: "\n", with: "\\n")
                            .replacingOccurrences(of: "\r", with: "\\r")
                        let js = "window.max._rejectFetch('\(safeCallId)', '\(errorMessage)')"
                        await MainActor.run {
                            targetWebView?.evaluateJavaScript(js, completionHandler: nil)
                        }
                    }
            ...
            if let type = body["type"] as? String, type == "open_link" {
                ...
                // Sandbox: only allow the Max branding domain.
                if sandboxMode {
                    let host = url.host?.lowercased() ?? ""
                    guard host == "max.ai" || host.hasSuffix(".max.ai") else {
            ...
            if let type = body["type"] as? String, type == "confirm" {
                ...
                let safeId = confirmId
                    ...
                    .replacingOccurrences(of: "\n", with: "\\n")
                    .replacingOccurrences(of: "\r", with: "\\r")
                let js = "window.max._resolveConfirm('\(safeId)', \(confirmed))"
        ...
        func resolveDataResponse(_ response: AppDataResponseMessage) {
            ...
            let js = "window.max.data._resolve('\(safeCallId)', \(response.success), \(resultJson), \(errorStr))"
        ...
        /// Send a content update to the web view via window.max.onContentUpdate().
                ...
                (function() {
                    try {
                        if (typeof window.max !== 'undefined' &&
                            typeof window.max.onContentUpdate === 'function') {
                            var data = JSON.parse('\(safeJSON)');
                            window.max.onContentUpdate(data);
                        }
                    } catch(e) {
        ...
        func webView(
            ...
            if sandboxMode {
                // In sandbox mode, only allow maxapp:// and about:blank URLs
                if let url = navigationAction.request.url {
                    let scheme = url.scheme?.lowercased() ?? ""
                    if scheme == MaxAppSchemeHandler.scheme || url.absoluteString == "about:blank" {
                        decisionHandler(.allow)
                        return
                    }
                    // Allow initial HTML load via https://*.max.local/
                    if scheme == "https" && (url.host?.hasSuffix(".max.local") == true) && navigationAction.navigationType == .other {
                        decisionHandler(.allow)
                        return
                    }
                }
