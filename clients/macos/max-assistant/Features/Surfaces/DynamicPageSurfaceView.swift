...
extension DynamicPageSurfaceView {
    ...
    static let designSystemCSS: String = {
        guard let url = ResourceBundle.bundle.url(
            forResource: "max-design-system", withExtension: "css"
        ), let css = try? String(contentsOf: url, encoding: .utf8) else {
            log.error("Failed to load max-design-system.css from resource bundle")
            assertionFailure("max-design-system.css not found in resource bundle")
            return ""
        }
        return css
    ...
    static let widgetJS: String = {
        guard let url = ResourceBundle.bundle.url(
            forResource: "max-widgets", withExtension: "js"
        ), let js = try? String(contentsOf: url, encoding: .utf8) else {
            log.error("Failed to load max-widgets.js from resource bundle")
            assertionFailure("max-widgets.js not found in resource bundle")
            return ""
        }
        return js
    }()
        ...
        WKUserScript(
            source: """
                (function() {
                    ...
                    style.id = 'max-design-system';
                    style.setAttribute('data-max-injected', '1');
    ...
    static let editAnimatorUserScript: WKUserScript? = {
        guard let url = ResourceBundle.bundle.url(forResource: "max-edit-animator", withExtension: "js"),
              let js = try? String(contentsOf: url, encoding: .utf8) else {
            return nil
        }
    ...
    /// Blocks all network requests except maxapp:// and about:blank.
...
struct DynamicPageSurfaceView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSView {
        ...
        var jsSource = """
            (function() {
                ...
                function _fwd(level, args) {
                    try {
                        ...
                        window.webkit.messageHandlers.maxBridge.postMessage({
            ...
            window.max = {
                sendAction: function(actionId, data) {
                    window.webkit.messageHandlers.maxBridge.postMessage({actionId: actionId, data: data});
                },
                openExternal: function(url) {
                    window.webkit.messageHandlers.maxBridge.postMessage({type: 'open_external', url: String(url)});
                },
                ...
                confirm: function(title, message) {
                    return new Promise(function(resolve) {
                        var confirmId = 'confirm_' + (window.max._confirmNextId++);
                        window.max._confirmPending[confirmId] = resolve;
                        window.webkit.messageHandlers.maxBridge.postMessage({
                ...
                _resolveConfirm: function(confirmId, result) {
                    var p = window.max._confirmPending[confirmId];
                    if (p) { delete window.max._confirmPending[confirmId]; p(result); }
                }
            };
            """
                ...
                window.max.data = {
                    ...
                    _call: function(method, params) {
                        return new Promise(function(resolve, reject) {
                            var callId = 'c' + (window.max.data._nextId++);
                            window.max.data._pending[callId] = { resolve: resolve, reject: reject };
                            ...
                            window.webkit.messageHandlers.maxBridge.postMessage(msg);
                        });
                    },
                    ...
                    _resolve: function(callId, success, result, error) {
                        ...
                        if (!p) {
                            console.warn('[max.data] _resolve called for unknown callId:', callId);
                            return;
                        }
        ...
        // Inject window.max.fetch — an authenticated fetch wrapper that routes
        ...
        // https://*.max.local but the gateway runs on http://127.0.0.1).
        //
        ...
        if appId != nil, let credentials = GatewayHTTPClient.resolveWebViewCredentials() {
            ...
            log.info("[max.fetch] Bridge injected (native): baseURL=\(credentials.baseURL, privacy: .public) pathPrefix=\(credentials.pathPrefix, privacy: .public) headerKeys=[\(headerKeys, privacy: .public)] hasAuth=\(hasAuth, privacy: .public)")
            ...
            jsSource += """
                ...
                window.max._pendingFetches = {};
                window.max._resolveFetch = function(callId, statusCode, statusText, body) {
                    var p = window.max._pendingFetches[callId];
                    if (!p) return;
                    delete window.max._pendingFetches[callId];
                ...
                window.max._rejectFetch = function(callId, errorMessage) {
                    var p = window.max._pendingFetches[callId];
                    if (!p) return;
                    delete window.max._pendingFetches[callId];
                    p.reject(new Error(errorMessage));
                };
                window.max.fetch = function(path, options) {
                    options = options || {};
                    ...
                    console.log('[max.fetch] ' + method + ' ' + url + ' headers=[' + headerNames + '] (native bridge)');
                    return new Promise(function(resolve, reject) {
                        window.max._pendingFetches[callId] = { resolve: resolve, reject: reject };
                        window.webkit.messageHandlers.maxBridge.postMessage({
                            type: 'fetch_request',
                            callId: callId,
                            url: url,
                        ...
                        console.log('[max.fetch] ' + method + ' ' + url + ' → ' + res.status + ' ' + res.statusText);
                        return res;
                    }).catch(function(err) {
                        console.error('[max.fetch] ' + method + ' ' + url + ' FAILED: ' + (err.message || err));
                        throw err;
                    });
                };
                """
        } else {
            log.warning("[max.fetch] Credentials nil — fallback (always-reject) bridge installed")
            jsSource += """

                window.max.fetch = function() {
                    console.error('[max.fetch] REJECTED: assistant connection could not be resolved (credentials were nil at WebView creation)');
                    return Promise.reject(new Error('max.fetch is not available: assistant connection could not be resolved'));
                };
                """
        }
        ...
        jsSource += """
            ...
            document.addEventListener('DOMContentLoaded', function() {
                var hasData = !!(window.max && window.max.data);
                var hasFetch = !!(window.max && window.max.fetch);
                console.log('[max] Bridge check: max.data ' + (hasData ? 'available' : 'NOT available (appId not set)') + ', max.fetch ' + (hasFetch ? 'available' : 'NOT available'));
                if (hasFetch) {
                    try {
                        var testResult = window.max.fetch.toString().slice(0, 80);
                        console.log('[max.fetch] impl=' + testResult);
                    } catch(e) {}
                }
            });
            """
        ...
        contentController.add(context.coordinator, name: "maxBridge")

        let configuration = WKWebViewConfiguration()
        configuration.setURLSchemeHandler(
            MaxAppSchemeHandler(),
            forURLScheme: MaxAppSchemeHandler.scheme
        )
        ...
        if topContentInset > 0 || bottomContentInset > 0 {
            ...
            let insetScript = WKUserScript(
                source: """
                    (function() {
                        var style = document.createElement('style');
                        style.id = 'max-content-insets';
                        style.setAttribute('data-max-injected', '1');
                        style.textContent = 'body { padding-top: \(top)px; padding-bottom: \(bottom)px; }';
                        (document.head || document.documentElement).appendChild(style);
                        if (\(bottom) > 0) {
                            function setupFade() {
                                var fade = document.getElementById('max-bottom-fade');
                                if (!fade) {
                                    fade = document.createElement('div');
                                    fade.id = 'max-bottom-fade';
                                    fade.setAttribute('data-max-injected', '1');
        ...
        if let appId = appId {
            ...
            let origin = "https://\(appId).max.local/"
            ...
            let origin = "https://surface.max.local/"
            webView.loadHTMLString(data.html, baseURL: URL(string: origin))
        }
    ...
    func updateNSView(_ containerView: NSView, context: Context) {
        ...
        if let gen = data.reloadGeneration, gen != context.coordinator.lastReloadGeneration {
            ...
            if context.coordinator.isInlineFallback {
                ...
                let origin = appId.map { "https://\($0).max.local/" } ?? "https://surface.max.local/"
                webView.loadHTMLString(data.html, baseURL: URL(string: origin))
            } else {
                webView.reload()
            }
                ...
                Task { @MainActor in
                    do {
                        let value = try await webView.callAsyncJavaScript(
                            "return await window.max.morphWithAnimation(newHTML)",
        ...
        if newTop != context.coordinator.lastTopInset || newBottom != context.coordinator.lastBottomInset {
            ...
            let js = """
                (function() {
                    var el = document.getElementById('max-content-insets');
                    if (!el) { el = document.createElement('style'); el.id = 'max-content-insets'; el.setAttribute('data-max-injected', '1'); (document.head || document.documentElement).appendChild(el); }
        ...
        if let status = data.status, status != context.coordinator.lastStatus {
            ...
            let js = """
                (function() {
                    var existing = document.getElementById('max-status-pill');
                    if (existing) existing.remove();
                    var pill = document.createElement('div');
                    pill.id = 'max-status-pill';
                    pill.setAttribute('data-max-injected', '1');
    ...
    static func dismantleNSView(_ containerView: NSView, coordinator: Coordinator) {
        ...
        controller.removeScriptMessageHandler(forName: "maxBridge")
