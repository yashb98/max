...
extension AppDelegate {
    ...
    /// Managed assistants (cloud == "max") use platform proxy with session token auth.
    ...
    func setupGatewayConnectionManager() {
        ...
        connectionManager.wakeHandler = { [weak self] in
            ...
            try await self.maxCli.wake(name: name)
        }
        ...
        connectionManager.postSparkleUpdateHandler = { [weak self] name, fromVersion in
            guard let self else { return }
            try? await self.maxCli.upgradeFinalize(name: name, fromVersion: fromVersion)
        }
    ...
    func setupAutoUpdate() {
        ...
        updateManager.onWillInstallUpdate = { [weak self] in
            ...
            let backupPath = try? await self.maxCli.upgradePrepare(
                name: name,
    ...
    /// Installs a `/usr/local/bin/max` symlink pointing to the bundled
    /// CLI binary so users can run `max` from their terminal.
    ///
    /// Skipped when dev mode is active (developers manage their own PATH)
    /// or when `max` already resolves to a different executable
    ...
    nonisolated static func installCLISymlinkIfNeeded(isDevMode: Bool) {
        ...
        let cliBinary = macosDir.appendingPathComponent("max-cli")
        if FileManager.default.fileExists(atPath: cliBinary.path) {
            installSymlink(commandName: "max", target: cliBinary.path)
        }
