...
extension AppDelegate {
    ...
    public func application(_ application: NSApplication, open urls: [URL]) {
        for url in urls {
            // Handle max://send?message=... deep links
            if url.scheme == "max" || url.scheme == "max-assistant" {
                handleDeepLink(url)
                continue
            }

            guard url.pathExtension == "max" else { continue }
            log.info("Opening .max file: \(url.path, privacy: .public)")
    ...
    /// Handle `max://send?message=...` deep links by buffering the message
    ...
    func handleOpenBundleResponse(_ response: OpenBundleResponseMessage, filePath: String = "") {
        ...
        if response.manifest.format_version > 2 {
            ...
            alert.informativeText = "This app requires a newer version of max-assistant."
    ...
    func unpackAndLoadBundle(
        ...
        Task.detached {
            do {
                ...
                await MainActor.run {
                    // Build the maxapp:// URL for the entry point.
                    // Sanitize manifest.entry to prevent JS string breakout.
                    let sanitizedEntry = manifest.entry
                        .replacingOccurrences(of: "\\", with: "")
                        .replacingOccurrences(of: "'", with: "")
                    let entryURL = "\(MaxAppSchemeHandler.scheme)://\(uuid)/\(sanitizedEntry)"
