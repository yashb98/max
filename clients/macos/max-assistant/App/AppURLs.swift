...
///
/// The docs base URL can be overridden at runtime via the `MAX_DOCS_BASE_URL`
...
public enum AppURLs {
    /// Default base URL for Max docs. Used when no env var override is set.
    public static let defaultDocsBaseURL = "https://www.max.ai/docs"

    /// Base URL for Max docs. Honors `MAX_DOCS_BASE_URL` if set, otherwise
    ...
    public static var docsBaseURL: String {
        let raw = ProcessInfo.processInfo.environment["MAX_DOCS_BASE_URL"]?
    ...
    /// build environment.
    ///
    /// If `MAX_WEB_URL` is set but malformed (no scheme/host, non-http(s),
    /// or contains a query/fragment), falls back to the canonical environment
    /// URL via `MaxEnvironment.current.webURL`. Query/fragment are rejected
    ...
    public static var billingSettings: URL {
        let candidate = MaxEnvironment.resolvedWebURL
        let base: String
        if let url = URL(string: candidate),
           ...
           url.fragment == nil {
            ...
            base = MaxEnvironment.current.webURL
        }
    ...
    public static let repositoryURL = URL(string: "https://github.com/max-ai/max-assistant")!
    ...
    /// Max community hub — linked from the Settings "Community" tab.
    public static let communityHubURL = URL(string: "https://max.ai/community")!

    /// Twitter/X profile — linked from the Settings "Community" tab.
    public static let twitterURL = URL(string: "https://x.com/max_ai")!

    /// YouTube channel — linked from the Settings "Community" tab.
    public static let youtubeURL = URL(string: "https://www.youtube.com/@Max_AI")!
