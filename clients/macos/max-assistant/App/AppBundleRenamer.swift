...
///
/// Production builds always use "Max" so this is a no-op for them.
@MainActor
enum AppBundleRenamer {
    ...
    /// "Max" and a rename + relaunch is needed.
    ...
    static func renameAndRelaunch() -> Bool {
        ...
        // (production builds installed to /Applications/ are always "Max").
        ...
        # `MaxCli.stopTimeout` (15s) plus AppKit teardown headroom
        ...
        let scriptURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("max-rename-\(UUID().uuidString).sh")
