...
final class AppleContainersLauncher: AssistantManagementClient {
    ...
    func hatch(
        ...
        let imageRefs = MaxImageReference.defaults(version: "latest")
        ...
        if MaxEnvironment.current == .local {
        ...
        let platformURL: String = MaxEnvironment.current.platformURL
        ...
        // assistant immediately (e.g. `max ps`). The runtimeUrl is
    ...
    private func buildLocalImages(
        ...
        imageRefs: [MaxServiceName: MaxImageReference],
        onProgress: (@MainActor (String) -> Void)?
    ) async throws {
        guard let repoRoot = LocalImageBuilder.findRepoRoot() else {
            throw LauncherError.hatchFailed("No repo root found — cannot build images locally. Run from within the max-assistant repository.")
        }
    ...
    /// `$XDG_CONFIG_HOME/max/assistants/<id>/guardian-token.json`,
    ...
    private static func saveGuardianTokenFile(
        ...
        let dir = "\(configHome)/max/assistants/\(assistantId)"
    ...
    static func nextAvailableSocketPath() -> String {
        ...
        let root = appSupport
            .appendingPathComponent(MaxEnvironment.current.appSupportDirectoryName, isDirectory: true)
    ...
    private static func removeGuardianToken(assistantId: String) {
        ...
        let tokenDir = "\(configHome)/max/assistants/\(assistantId)"
