...
private let log = Logger(subsystem: Bundle.appBundleIdentifier, category: "MaxCli")
...
@MainActor
final class MaxCli: AssistantManagementClient {
    ...
    /// `MAX_ENVIRONMENT` must be forwarded so the bundled CLI resolves
    ...
    private var cliBinaryURL: URL? {
        ...
        let candidate = execURL.deletingLastPathComponent().appendingPathComponent("max-cli")
    ...
    /// assistant entry. Does NOT delete ~/.max (macOS app manages its data).
    ///
    ...
    override func retire(name: String? = nil) async throws -> LockfileAssistant? {
        ...
        let (stderr, status) = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<(String, Int32), Error>) in
            ...
            let env = MaxCli.makeBaseEnvironment()
    ...
    /// Stops the daemon and gateway processes via `max sleep --force`.
    ...
    nonisolated func stop(name: String? = nil) async {
        ...
        let binaryURL = execURL.deletingLastPathComponent().appendingPathComponent("max-cli")
    ...
    func runRemoteHatch(
        ...
        if config.remote == "docker", env["MAX_PLATFORM_URL"] == nil {
            env["MAX_PLATFORM_URL"] = MaxEnvironment.current.dockerHostPlatformURL
        }
        ...
        if config.remote == "gcp" {
            ...
            if !config.gcpServiceAccountKey.isEmpty {
                let tmpKeyPath = FileManager.default.temporaryDirectory
                    .appendingPathComponent("max-sa-key-\(ProcessInfo.processInfo.processIdentifier).json")
            ...
            if !config.awsRoleArn.isEmpty {
                env["MAX_AWS_ROLE_ARN"] = config.awsRoleArn
            }
        }
        ...
        let bufferQueue = DispatchQueue(label: "com.max.cli.line-buffer")
    ...
    private func runCLI(
        ...
        do {
            result = try await Task.detached {
                ...
                proc.environment = MaxCli.makeBaseEnvironment()
