...
extension AppDelegate {
    ...
    /// Returns `true` when `~/.max.lock.json` contains at least one
    ...
    func showOnboarding() {
        ...
        onboarding.onComplete = { [weak self] state in
            ...
            // or authenticated via Max Account (WorkOS). Proceed directly —
...
extension AppDelegate {
    ...
    public func showAboutPanel() {
        ...
        let aboutView = AboutMaxView(connectionManager: connectionManager)
