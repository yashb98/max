...
/// Client-resolved max:* identity fields that must be POSTed to a
...
/// assistant's CES has no `max:platform_base_url` entry — the in-memory
...
enum ManagedAssistantIdentityInjection {
    /// Inject the client-resolvable `max:platform_base_url`,
    /// `max:platform_organization_id`, and `max:platform_user_id`
    ...
    /// `MAX_PLATFORM_URL` fallback until the next explicit set.
    static func inject(
        ...
        await GatewayHTTPClient.withAssistant(assistantId) {
            await postSecret(
                ...
                name: "max:platform_base_url",
                value: MaxEnvironment.resolvedPlatformURL
            )
            await postSecret(
                assistantId: assistantId,
                name: "max:platform_organization_id",
                value: organizationId
            )
            if let userId, !userId.isEmpty {
                await postSecret(
                    assistantId: assistantId,
                    name: "max:platform_user_id",
                    value: userId
                )
            } else {
