...
public final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// and tooltips.  For CI builds this is either "Max" (production) or
    /// "Max Staging" (staging).  Local dev builds where
    /// `BUNDLE_DISPLAY_NAME` is a custom assistant name (e.g. "Jarvis") fall
    /// back to "Max" so menus and the About panel always show the brand.
    public static let appName: String = {
        let display = Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as? String
            ...
            ?? "Max"
        // CI sets BUNDLE_DISPLAY_NAME to "Max" or "Max Staging".
        // Local dev builds may set it to a custom assistant name.  Only
        // recognise values that start with "Max" as valid brand names.
        return display.hasPrefix("Max") ? display : "Max"
    }()
    ...
    /// Tracks file paths of .max bundles awaiting assistant responses (FIFO).
    ...
    /// Whether the current assistant is platform-managed (cloud == "max").
    ...
    /// Delegate that patches the app menu items to "Max" right before
    /// macOS renders them.
    ...
    public func applicationWillFinishLaunching(_ notification: Notification) {
        // Ensure the macOS app menu consistently says "Max" (Hide Max,
        // Quit Max, etc.) regardless of the executable name — which may
        // differ between production builds (renamed to "Max" by build.sh)
    ...
    /// "Max".  The menu bar title is patched via didBeginTracking (fires
    ...
    public func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if sendDiagnostics && !MetricKitManager.macosDSN.isEmpty {
            ...
            let commitSHA = Bundle.main.infoDictionary?["MaxCommitSHA"] as? String
            nonisolated(unsafe) var crashedLastRun = false
            SentrySDK.start { options in
                options.dsn = MetricKitManager.macosDSN
                options.releaseName = "max-macos@\(appVersion)"
