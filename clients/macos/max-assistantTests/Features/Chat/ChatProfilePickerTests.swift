...
final class ChatProfilePickerTests: XCTestCase {
    ...
    // MARK: - ComposerSettingsMenu: kimi-agent unavailable branch

    func test_kimiAgentTrailingText_missingCli() {
        let text = ComposerSettingsMenu.kimiAgentTrailingText(reason: .missingCli)
        XCTAssertEqual(text, "Install Kimi CLI")
    }

    func test_kimiAgentTrailingText_notLoggedIn() {
        let text = ComposerSettingsMenu.kimiAgentTrailingText(reason: .notLoggedIn)
        XCTAssertEqual(text, "Sign in to Kimi")
    }

    func test_kimiAgentTrailingText_notEnabled() {
        let text = ComposerSettingsMenu.kimiAgentTrailingText(reason: .notEnabled)
        XCTAssertEqual(text, "Feature flag off")
    }

    func test_kimiAgentTrailingText_noApiKeyFallback() {
        let text = ComposerSettingsMenu.kimiAgentTrailingText(reason: .noApiKey)
        XCTAssertEqual(text, "Not available")
    }

    func test_kimiAgentRowLabel_missingCli() {
        let label = ComposerSettingsMenu.kimiAgentRowLabel(reason: .missingCli)
        XCTAssertEqual(label, "Kimi (Agent SDK) · not installed")
    }

    func test_kimiAgentRowLabel_notLoggedIn() {
        let label = ComposerSettingsMenu.kimiAgentRowLabel(reason: .notLoggedIn)
        XCTAssertEqual(label, "Kimi (Agent SDK) · not signed in")
    }

    func test_kimiAgentRowLabel_notEnabled() {
        let label = ComposerSettingsMenu.kimiAgentRowLabel(reason: .notEnabled)
        XCTAssertEqual(label, "Kimi (Agent SDK) · disabled")
    }

    func test_kimiAgentRowLabel_availableFallsBack() {
        let label = ComposerSettingsMenu.kimiAgentRowLabel(reason: nil)
        XCTAssertEqual(label, "Kimi (Agent SDK)")
    }

    // MARK: - ComposerSettingsMenu: providerGroups kimi-agent surfacing

    func test_providerGroups_kimiAgentEmptySurfacedWhenFlagged() {
        let groups = ComposerSettingsMenu.providerGroups(
            profiles: [],
            connectionReachability: [:],
            includeEmptyKimiAgent: true
        )
        XCTAssertTrue(groups.contains { $0.kind == .kimiAgent })
    }

    func test_providerGroups_kimiAgentHiddenByDefault() {
        let groups = ComposerSettingsMenu.providerGroups(
            profiles: [],
            connectionReachability: [:]
        )
        XCTAssertFalse(groups.contains { $0.kind == .kimiAgent })
    }

    // MARK: - ComposerSettingsMenu: kimi-agent model label

    func test_kimiAgentModelLabel_stripsAgentSuffix() {
        let label = ComposerSettingsMenu.kimiAgentModelLabel("Kimi K2.6 (agent)")
        XCTAssertEqual(label, "Kimi K2.6")
    }

    func test_kimiAgentModelLabel_leavesUnsuffixedUnchanged() {
        let label = ComposerSettingsMenu.kimiAgentModelLabel("Kimi K2.6")
        XCTAssertEqual(label, "Kimi K2.6")
    }
}
