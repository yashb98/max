...
final class LockfileAssistantManagedTests: XCTestCase {
    ...
    override func setUp() {
        ...
        lockfilePath = tempDir.appendingPathComponent(".max.lock.json").path
    }
    ...
    func testEnsureInsertsWhenLockfileDoesNotExist() {
        ...
        XCTAssertEqual(assistants[0]["cloud"] as? String, "max")
        XCTAssertEqual(assistants[0]["runtimeUrl"] as? String, "https://platform.max.ai")
    ...
    // MARK: - ensureManagedEntry: always sets cloud to "max"

    func testEnsureAlwaysSetsCloudToMax() {
        LockfileAssistant.ensureManagedEntry(
            assistantId: "test-id",
            runtimeUrl: "https://platform.max.ai",
            hatchedAt: "2024-01-01T00:00:00Z",
    ...
    func testIsManagedReturnsTrueForMaxCloud() {
    ...
    func testIsRemoteReturnsTrueForMax() {
        let assistant = LockfileAssistant(
            assistantId: "test-id",
            runtimeUrl: "https://platform.max.ai",
            bearerToken: nil,
    ...
    // MARK: - home property for max cloud

    func testHomeReturnsMaxVariantForManagedAssistant() {
        let assistant = LockfileAssistant(
            assistantId: "test-id",
            runtimeUrl: "https://platform.max.ai",
            bearerToken: nil,
        ...
        if case .max(let runtimeUrl) = assistant.home {
            XCTAssertEqual(runtimeUrl, "https://platform.max.ai")
        } else {
            XCTFail("Expected .max home for managed assistant, got \(assistant.home)")
        }
    }
