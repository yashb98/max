...
final class GuardianTokenFileReaderTests: XCTestCase {
    ...
    /// Per-env `MaxPaths` rooted at this test's tmp dir so the sweep
    /// touches only the test sandbox, never the user's real `~/.config/`.
    private func makeEnvPaths() -> [MaxPaths] {
        MaxPaths.allEnvs(
    ...
    private func plantTokenFile(at paths: MaxPaths, assistantId: String) {
    ...
    private func tokenFilePath(at paths: MaxPaths, assistantId: String) -> String {
        paths.configDir
            .appendingPathComponent("assistants")
            .appendingPathComponent(assistantId)
    ...
    /// next `max wake`, silently undoing the re-pair.
    func testDeleteAcrossAllEnvsRemovesSiblingCopies() {
        let assistantId = "max-test-pike-abc123"
        let envPaths = makeEnvPaths()
        let plantedEnvs: [MaxEnvironment] = [.production, .dev, .local]
    ...
    func testDeleteAcrossAllEnvsLeavesOtherAssistantsAlone() {
        let target = "max-target-pike"
        let bystander = "max-bystander-deer"
        let envPaths = makeEnvPaths()
        let plantedEnvs: [MaxEnvironment] = [.production, .dev]
        for paths in envPaths where plantedEnvs.contains(paths.environment) {
