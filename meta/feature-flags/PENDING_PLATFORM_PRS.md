...
LaunchDarkly/Terraform entry in the `max-assistant-platform` repo. Per
`CLAUDE.md` (see the Assistant Feature Flags section) and
`meta/feature-flags/AGENTS.md`, a new flag in this registry requires a
companion PR in `max-assistant-platform` to provision the flag on the
...
|---|---|---|---|---|
| `meet` | 2026-04-19 | sidd@max.ai | Platform PR not yet opened (as of 2026-04-19) | Terraform entry in `../max-assistant-platform/terraform/launchdarkly.tf` (or equivalent) with `defaultEnabled: false` and a description pointing at `skills/meet-join/SKILL.md`. |
